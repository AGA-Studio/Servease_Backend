...
from servicios.serializers import ServicioListSerializer, ServicioSerializer
...
EMAIL_SEND_ERROR = "No pudimos enviarte el correo de confirmación. Intenta de nuevo."
...
class HomeClienteView(ListAPIView):
    ...
    def get_queryset(self):
        return VistaHomeCliente.objects.filter(cliente_id=self.kwargs["id_usuario"])
...
class UltimasPublicacionesClienteView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if str(self.request.user.id_usuario) != str(id_usuario):
            raise PermissionDenied("No puedes ver las publicaciones de otro usuario.")
...
class SignupView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"detail": SIGNUP_SUCCESS_MESSAGE}, status=status.HTTP_201_CREATED
        )
...
# ──────────────────────────────────────────────
#  Dev-only login (DEBUG=True, no real Supabase)
# ──────────────────────────────────────────────
class DevLoginView(APIView):
    """POST /api/usuarios/dev-login/
    Only active when DEBUG=True and Supabase keys are placeholders.
    Authenticates against local DB, returns a JWT signed with simplejwt.
    """
    ...
    def post(self, request):
        from .serializers import DevLoginSerializer
        ...
        token = AccessToken()
        token["sub"] = str(usuario.id_usuario)
