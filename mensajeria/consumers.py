...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # Mark incoming messages from the other participant as 'recibido'
        # BEFORE accepting: when the client receives the connect confirmation,
        # pending messages must already be in their final delivery state.
        await self._mark_delivered(user)
    ...
    @sync_to_async(thread_sensitive=False)
    def _mark_delivered(self, user):
        """Flip 'enviado' to 'recibido' for messages sent by the other participant."""
        return (
            ...
            .exclude(emisor=user)
            .update(estado_entrega="recibido")
        )
